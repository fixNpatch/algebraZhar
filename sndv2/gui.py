...
class Engine(Frame):
    ...
    def initUI(self):
        ...
        key_entry = Entry(textvariable="", width=150, name="entry_q")
    ...
    def Crypt(self):
        ...
        chosen_key = alphabet[key]
        messagebox.showinfo("GenKey", chosen_key)

        text_box = self.parent.nametowidget('text_box_q')

        result = MainCrypt(text_box.get(0.0, END), key)
        text_box.delete(0.0, END)
    ...
    def DeCrypt(self):
        ...
        result = MainDeCrypt(text_box.get(0.0, END), str(key_entry.get()))
        text_box.delete(0.0, END)
        text_box.insert(0.0, result)
